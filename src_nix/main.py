...
# from pltsurf import plotSurf
from expsim import simulateExperiment
...
"""
# Plot plohe svih eksperimenata
(minZalPltArr, maxZalPltArr, pzptPltArr, minZalArr, maxZalArr, pzptArr,
 pocZalArr, zavZalArr, izdNarArr, velNarArr) = plotSurf(
    potr, maxZalD, maxZalG, minZalD, minNar, daniDob, pzptd, pzptg, pltDwnSmp
)
"""
# print('Pokretanje eksperimenta Cython')
# simulateExperiment(potr, pocZalExp, maxZalD, maxZalG, minZalD, minZalG, minNar,
#                    daniDob, radVrDob, radVrSub, 0, pzptDPerc,
#                    pzptGPerc, pzptProChk, pzptDanChk, izlFol, sprNaj, algType,
#                    diagStopPerc)

# print('Vrijeme izvršavanja Cython: {:.4f} s'.format(time.time() - startTimeCy))

if __name__ == '__main__':
    mp.freeze_support()
    mp.set_start_method('spawn')
    print('Pokretanje eksperimenta Numba')
    OptimInv = OIClass()
    OptimInventoryApp().run()
    # OptimInv = OIClass(
    #     ulFol, izlFol, pocZalExp, maxZalD, maxZalG, minZalD, minZalG, minNar,
    #     daniDob, radVrDob, radVrSub, blChk, blDPerc, blGPerc, pzptProDPerc,
    #     pzptProGPerc, pzptDanDPerc, pzptDanGPerc,  pzptProChk, pzptDanChk,
    #     velDobChk, velDobD, velDobG, sprNaj, diagStopPerc, max_proc, proc_num,
    #     email_chk, recipient, sender, smtp_server, password
    # )
    # OptimInv.startTimeCount()
    # OptimInv.evaluateParallel(findOptExp)
    # OptimInv.sendEmail()
    # print('Vrijeme izvršavanja Numba: {} s'.format(OptimInv.exp_time))

"""
pocZal = np.zeros_like(potr, dtype=np.int32)
zavZal = np.zeros_like(potr, dtype=np.int32)
izdNar = np.zeros_like(potr, dtype=np.bool_)
velNar = np.zeros_like(potr, dtype=np.int32)
maxZalPltArr = []
minZalPltArr = []
pzptPltArr = []
pzptArr = []
    ...
    for minZal in range(minZalD, maxZal):
        ...
        for dan in range(0, potr.size):
            ...
            if zavZal[dan] <= minZal:
                ...
                izdNar[dan] = 0
                velNar[dan] = 0
        pzpt = (pocZal - zavZal).sum() / potrSum
...
maxZalPltArr = np.array(maxZalPltArr)
minZalPltArr = np.array(minZalPltArr)
pzptPltArr = np.array(pzptPltArr)

pzptArr = np.array(pzptArr)
minZalArr = np.array(minZalArr)
maxZalArr = np.array(maxZalArr)
pocZalArr = np.array(pocZalArr)
zavZalArr = np.array(zavZalArr)
izdNarArr = np.array(izdNarArr)
velNarArr = np.array(velNarArr)
csvArr = np.stack((pocZalArr, potr, zavZalArr, izdNarArr, velNarArr), axis=-1)
infoTxt = 'PZPT: {}, MAX: {}, MIN: {}, DD: {}, DS: {}'.format(
    pzptArr[0], maxZalArr[0], minZalArr[0], daniDob, potr.size)

np.savetxt(
    izlFol+'/PZPT{}_MAX{}_MIN{}_DD{}_DS{}.csv'.format(
        pzptArr[0], maxZalArr[0], minZalArr[0], daniDob, potr.size),
    csvArr, fmt='%1u', delimiter=';', newline='\n', header=infoTxt
)

print('Broj tocaka za plot povrsine: {}'.format(pzptPltArr.size))
print('Broj tocaka za plot zadovoljene PZPT: {}'.format(pzptArr.size))
print(infoTxt)


fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_trisurf(minZalPltArr, maxZalPltArr, pzptPltArr,
                cmap=cm.viridis, alpha=0.9,
                label='PZPT (proizvodi) = f(minZal, maxZal)')
#ax.plot(minZalPltArr, maxZalPltArr, pzptPltArr, alpha=0.8,
#        label='PZPT (proizvodi) = f(minZal, maxZal)')
ax.scatter(minZalArr, maxZalArr, pzptArr, c='r', marker='.')
ax.set_xlabel('Minimalna razina zaliha')
ax.set_ylabel('Maksimalna razina zaliha')
ax.set_zlabel('PZPT (proizvodi)')
plt.show()
"""
